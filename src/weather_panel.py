...
def draw_precip(img, info, pos_x, pos_y, face_map):
    ...
    for i, label in enumerate(['早朝', '午前', '午後', '夜']):
        ...
        value_pos_x = pos_x + face['label'].getsize('午前')[0] + face['value'].getsize('100')[0]
        unit_pos_y = pos_y + face['value'].getsize('0')[1] - face['unit'].getsize('%')[1]
...
def draw_weather(img , label, info, pos_x, pos_y, face_map):
    next_pos_x = pos_x + 10
    next_pos_y = draw_text(img, label, [next_pos_x, pos_y],
                           face_map['weather']['day'], color='#999')[1]
    next_pos_x = draw_icon(img, info, next_pos_x, next_pos_y, face_map)
    next_pos_x = draw_temp(img, info, next_pos_x + 60, pos_y +50, face_map)
    next_pos_x = draw_precip(img, info, next_pos_x + 120, pos_y + 50, face_map)
...
def draw_date(img, pos_x, pos_y, face_map):
    ...
    draw_text(img, now.strftime('%H:%M'), [1050, pos_y], face['day'], color='#666', align='right')
...
def draw_panel_weather(img, config, font_config, weather_info):
    ...
    next_pos_x = draw_weather(img, 'Today', weather_info['today'], 10, 100, face_map)
    next_pos_x = draw_weather(img, 'Tommorow', weather_info['tommorow'], 10, 470, face_map)
